...
class ImageProcessor:
    ...
    def create_line_image(self, img):
        ...
        for row in range(line_image.shape[0]):
            for col in range(line_image.shape[1]):
                ...
                x = int(CIRCLE_CENTER[0] + rho * math.cos(theta) + 0)
                y = int(CIRCLE_CENTER[1] - rho * math.sin(theta) + 0)
    ...
    def process_image(self, denoise_path):
        ...
        # 去噪
        denoised_image = cv2.medianBlur(output, 3)
        # 高斯模糊
        # image_blur = cv2.GaussianBlur(denoised_image,  (5, 5), 1)

        # 膨胀
        dilated_image = cv2.dilate(denoised_image, np.ones((2, 2), np.uint8), iterations=1)
        # eroded_image = cv2.erode(dilated_image,  np.ones((2, 2), np.uint8), iterations=1)

        # 图像增强
        enhanced_image = cv2.convertScaleAbs(dilated_image, alpha=0.7, beta=3)
        ...
        sharpen_kernel = np.array([[-1, -1, -1],
                                   ...
                                   [-1, -1, -1]])
        # 使用卷积操作来锐化图像
        sharpen_image = cv2.filter2D(enhanced_image, -1, sharpen_kernel)

        # 二值化
        # threshold_value = 100
        # max_value = 150
        # ret, binary_image = cv2.threshold(enhanced_image, threshold_value, max_value, cv2.THRESH_BINARY)

        cv2.imshow('Binary Image', enhanced_image)
        cv2.waitKey(0)
        ...
        result = self.rapidocr_detect(enhanced_image)
