# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import detection_pb2 as detection__pb2


class DetectServiceStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.DetectFeaturesYolo = channel.unary_unary(
                '/com.yimuzn.featuresdetection.device.grpc.DetectService/DetectFeaturesYolo',
                request_serializer=detection__pb2.DetectRequest.SerializeToString,
                response_deserializer=detection__pb2.DetectResponse.FromString,
                )


class DetectServiceServicer(object):
    """Missing associated documentation comment in .proto file."""

    def DetectFeaturesYolo(self, request, context):
        """软件给算法发送图片，返回检测到的特征数量
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_DetectServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'DetectFeaturesYolo': grpc.unary_unary_rpc_method_handler(
                    servicer.DetectFeaturesYolo,
                    request_deserializer=detection__pb2.DetectRequest.FromString,
                    response_serializer=detection__pb2.DetectResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'com.yimuzn.featuresdetection.device.grpc.DetectService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class DetectService(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def DetectFeaturesYolo(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/com.yimuzn.featuresdetection.device.grpc.DetectService/DetectFeaturesYolo',
            detection__pb2.DetectRequest.SerializeToString,
            detection__pb2.DetectResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
